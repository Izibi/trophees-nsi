...
/***/ "./resources/js/UI/project-editor.js":
/*!*******************************************!*\
  !*** ./resources/js/UI/project-editor.js ***!
  \*******************************************/
/***/ (() => {
...
window.ProjectEditor = function (options) {
  ...
  inp_description.bind('input propertychange', refreshDescriptionCounter); // file inputs
  ...
  $('.link-delete-file').on('click', function (e) {
    ...
    var text = el.text();
    el.closest('.file-box').append($('<input type="hidden">').attr('name', 'delete_uploads[]').val(file));
    el.closest('.custom-file-controls').text(text);
  }); // team members

  function toggleTeamMembersHeader() {
    $('#team-members-header').toggle($('#team-members').children().length > 0);
  }
  ...
  toggleTeamMembersHeader(); // submit
...
__webpack_require__(/*! ./UI/project-editor */ "./resources/js/UI/project-editor.js");
